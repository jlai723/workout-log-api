...
router.get('/', validateJWT, async(req, res) => { // gets all logs for an individual user
    try {
        ...
        res.status(200).json({
            message: 'Workout log(s) successfully found',
            logEntries
        })
    } catch (err) {
        res.status(500).json({
...
router.get('/:id', validateJWT, async(req, res) => { // gets individual logs by id for an individual user
    try {
        ...
        if (logEntry) {
            res.status(200).json({
                message: 'Workout log successfully found',
                logEntry
            })
        } else {
            res.status(404).json({
...
router.put('/:id', validateJWT, async(req, res) => { // allows individual logs to be updated by a user
    try {
        ...
        if (update != 0) {
            res.status(200).json({
                message: `Workout log #${logId} successfully updated`,
                update,
                updatedLog
            })
        } else {
...
router.delete('/:id', validateJWT, async(req, res) => { // allows individual logs to be deleted by a user
    try {
        ...
        if (deleted != 0) {
            res.status(200).json({
                message: `Workout log #${logId} successfully deleted`,
                deleted
            })
        } else {
            res.status(404).json({
